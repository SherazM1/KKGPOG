...
from app.sams_club.image_resolution import SOURCE_LOCAL_ITEM_NUMBER, SOURCE_LOCAL_UPC
...
class SamsExcelUpcTests(unittest.TestCase):
    ...
    def test_fallback_resolution_by_item_number(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            workbook = root / "source.xlsx"
            image_dir = root / "images"
            image_dir.mkdir()
            self._write_workbook(workbook, "UPC", "", item_number="98765")
        ...
        self.assertEqual(result.debug["records_with_upc"], 0)
        self.assertEqual(result.debug["records_without_upc"], 1)
