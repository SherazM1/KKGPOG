...
def _draw_image_area(
    ...
    placeholder_text: str = "Image unavailable",
) -> None:
    c.setStrokeColorRGB(0.86, 0.86, 0.86)
    ...
    if image is None:
        if placeholder_text:
            ...
            c.drawCentredString(x + (w / 2), y + (h / 2) - 3, placeholder_text)
        return
...
def _draw_blank_slot(c: canvas.Canvas, x: float, y: float, w: float, h: float) -> None:
...
def _draw_slot_card(
    ...
    if slot.intentional_blank:
        _draw_blank_slot(c, x, y, w, h)
        return
    ...
    placeholder_text = "" if slot.gci_pending_image else "Image unavailable"
    _draw_image_area(c, inner_x, image_y, inner_w, image_h, image, placeholder_text)
...
def _render_side_page(
    ...
    for row_index, row in enumerate(sorted_rows):
        ...
        for slot_index, slot in enumerate(sorted_slots):
            ...
            if image is None and not slot.intentional_blank and not slot.gci_pending_image:
