...
def _resolve_ticket_positions_from_profile(
    ...
    if len(centers) < 2:
        warnings.append(
            f"JSON slot centers count {len(centers)} is not enough to define ticket boundaries; "
    ...
    if ticket_count == 1:
        ticket_centers = [(start_center + end_center) / 2.0]
    else:
        spacing = (end_center - start_center) / (ticket_count - 1)
        ticket_centers = [
            start_center + (idx * spacing)
            for idx in range(ticket_count)
        ]
    ...
    warnings.append(
        f"POG={row_data.pog}; Side={row_data.side}; Row={row_data.row}; "
        f"Strip width={strip_w}; Profile centers={len(centers)}; "
        f"Rendered tickets={ticket_count}; Spacing mode=dynamic"
    )
    return positions
...
def _holiday_description_lines_fit(
    lines: tuple[str, str, str],
    font_size: float,
    max_width: float,
) -> bool:
    return all(
        _estimate_text_width(line, font_size, "semibold" if index == 0 else "regular") <= max_width
...
def fit_holiday_description_text(
    ...
    if _holiday_description_lines_fit(normal_lines, font_size, description_width):
    ...
    for candidate_font_size in font_sizes:
        for candidate_left, candidate_top, candidate_width in candidates:
            ...
            if max_width < 8.0:
                continue
            if _holiday_description_lines_fit(normal_lines, candidate_font_size, max_width):
                return _HolidayDescriptionFit(normal_lines, candidate_font_size, safe_left, candidate_top, max_width)
